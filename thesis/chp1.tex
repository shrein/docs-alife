...
Biped walking is, from a robotics standpoint, the
...
skeletal muscles in human body, each one composed of several muscle
...
\begin{figure}
  ...
  \end{center}
  \caption{Static Equilibrium by CoM vertical projection over support
    polygon.}
...
supporting polygon, destabilizing moments due to dynamic locomotion
...
spinal cord inter-neurons acting as rhythmic central pattern
generators, neural system training along with bio-mechanical system and
...
Neural fields arise as a tissue level model of neural
populations in brain. It has been proposed by Wilson and Cowan
...
output function which is monotonically non decreasing. The deviation
of the average stimulation potential at place $x$ at time $y$ of layer
...
\cite{Wilson72Excitatory}. Nonetheless, while not stated otherwise it
will not taken into account the time lag, as well as the multiple
layers. The non-homogeneous terms will also be merged
...
For further simplification, temporarily the connection kernel
$w(x,x')$ will be considered as isotropic and homogeneous, so that it
...
% Also, the typical sigmoid saturation function, as well as its
% maximum gain limit case (the Heaviside function), are shown in the
% figure \ref{fig:sat-fun}.
...
Evolutionary algorithms are a set of population-based
heuristic search and optimization techniques. They maintain a
population, and apply a set of operators or transformations over its
...
\begin{figure}
  \begin{center}
    \includegraphics[width=1\textwidth]{FigTaxonomyTree}
  \end{center}
...
transitions, and probably other techniques such as Poincaré maps and
...
emphasizes the way in which the problem is solved and not the sub-field
...
arrangement is the Amari-Hopfield model, for which a graphical
...
without any sensory feedback. Later, they add a sensory entrance of
...
S-expression, doing a co-evolution where a sub-population for each
...
genetic algorithms (GA) to three morphological parameters. It is shown
...
used, like in previous works of Paul, co-evolution of some few
morphological parameters. This is probably the simpler solution to this
...
faced by them, the movement of the robot is restricted to sagital
...
And neuron states in $n+1$ are a function of recurrent
...
The function $\varphi()$ is a diagonal activation function,
usually of sigmoid shape. This way, a recurrent neural network is a
...
dynamics (an elementary behavior parametrized by behavior variables
...
the relative little depth with which the morphological evolution (and in
general the morphological configuration) for obtaining gait patterns has
...
redistributed, with the purpose of evaluating the morphological
...
obtained. The network is dynamically trained using back-propagation
trough time.
...
estimator that also consist of a feed-forward neural network, and a
...
training used is delayed back-propagation. The first step made is the
identification of the system with the neural estimator. Later, the
neural controller trains so that it follows a preassigned trajectory
...
order to obtain its objective, they use a set of feed-forward neural
...
adjustment is dynamically made on-line providing adaptability to the
...
Previously, on the same sense of previous work made by them, Wu et
...
inversely model the system. Also a feed-forward control with one neural
...
is updated dynamically (on-line) using as reinforcement signal some
...
obtained which uses the neural fuzzy networks for the control of the
...
H\"{u}lse et al. \cite{Huelse04Structure}, aimed to show that minimum
and robust control structures can be developed using algorithm ENS3
...
to luminance tropism, the morphological and control evolution for a
...
controllers to conserve robot stability according to dynamic stability
...
sagital plane, as well as a PD position control with inhibition from
the sagital plane oscillators. Also they applied the method to biped
...
later are transferred to the robot, which can make fine adjustments or
...
in which an notable on-line failure recovery strategy is implemented,
