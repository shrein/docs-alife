...
and scalar product, as shown in equations \ref{eq:eq-leb-opers} and \ref{eq:eq-leb-opers2}:
\begin{align}
...
Nonetheless, the simple biped model used was a first approach to the
problem of biped walking. In order to develop a more realistic
controller of biped walking, the mapping between the model and a biped
could be detailed \cite{Goswami08Kinematic}, or a relatively more
complex model could be used, such a 2-link model
...
control tasks that require, for example, multiple operation modes or control
strategies.
