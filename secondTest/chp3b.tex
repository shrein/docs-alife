...
be used onwards as the reference control problem. Optimal linear
state-feedback controllers for the Simplest Biped Walking problem are
found using numerical methods (genetic algorithms), one controller for
each sub-region on a delimited state-space region. A general
architecture for the neural field controllers is presented, and an
structured strategy to implement non-linear (sliding-mode) state-space
controllers using the neural field controller architecture is
shown. Finally, an specific controller for biped walking is developed
and tested using the collected data, its behaviour is shown using a
Poincarè section for the controlled biped dynamics, and an stability
...
The 2D Biped Walking (yet to be simplified) model asumes a biped with
...
another one in pendular motion (swing leg). The colission of the
swinging foot with the ground at heelstrike is plastic (no-slip,
...
ground at any time. For simplicity, it is asumed that the swing leg is
...
colission (otherwise the foot-scuffing problems inevitable for a
...
model, by asuming that the point masses located at the feet are
...
Furthemore, we now define the action torque vector as:
...
vector before and after colission. As shown by Garcia et al., there is
a phase reduction in the colission, from four dimensions to two
...
To complete the model, it is introduced the heelstrike event condition
...
Given that there is a model reduction on each heelstrike, each step is
uniquely determined by the initial state after heelstrike
$v^+=[\theta^+ \dot{\theta^+}]^T$. The stability of a given controller
can be analysed through the evolution of the Poincaré section $v^+_n$
after the $n$-th heelstrike. This is analogous to analysing the
...
sytem. As there are some $v$ values for which the stride function
...
wheel attains asympotically stable steady 2D motions, given the proper
ground slope and angle between legs \cite{Coleman97Motions}. Also, it
has the advantage that, for sufficiently small mass ratio $\beta$, the
energy cost of the required control action is negligible. Such control
strategy can be stated as
$T'_{\phi}=k_{\phi}(\phi_r-\phi)+\sqrt{k_{\phi}}\dot{\phi}$, where the
reference swing leg angle is set to a convinient value $\phi_r=0.3$.
...
have the bounds $\theta_0 = [0, 0.4]$ and $\dot{\theta}_0 = [-0.4:0]$,
with step size $\delta v=0.025$, for a 17x17 nodes grid. For each node
in the grid, a search with an evolutionary algorithm is performed,
using $k_{\phi}$ as the genotype. The fitness function evaluation
implies running a simulation of the model with the state-feedback
controller using the $k_{\phi}$ given, for a fixed time interval. The
simulation may abort before the fixed time, if state variable (not the
Poincaré section) $\theta(t)$ leaves the interval $[-\pi/2, \pi/2]$,
and the lowest fitness value is assigned. Otherwise, the fitness value
is $k_{\phi}^2$.
...
\begin{figure}[ht]
  ...
  \caption{Search result for the mapping $k_{\phi}=M(\theta_0,\dot{\theta_0})$.}
\end{figure}
...
Simplest Biped Walking model implemented with neural fields, a general
architecture for neural field controllers is presented.
...
neural field controllers to help the understanding of commonalities
...
described with an enfasis in how information is processed in the
controller.

The neural field controller architecture will be generally structured
as follows. It will have three layers: one input layer, one processing
layer and one output layer. As a strongly layered architecture, the
...
composed of neural populations, in terms of structure and behavior
...
variable (usually measured as an output of the controlled system) to
the activation potential of its neural population. Finally, each
element in the output layer consist only of a mapping function
from the activation potential of a processing layer population to the
value of an output variable. The layers are further described below:
...
as a buffer to feed the inputs to the processing layer populations,
and correspond in each iteration to the mapping of an input
variable. While the mapping function can be general, here it will be
simplified. As a result, it is used one input layer population for
...
not governed by a differential equation and therefore does not follow
...
$u_i(\theta,t) \in \mathbb{R}$ to each position $x$ in the input layer
population, at the current time $t$.
...
function has access to the entire history of $e_i$. In the simplest
case, the mapping function will depend only on the current value
$e_i(t)$, as the case in the controller developed in this chapter.
...
different poopulations $i$ and $j$), $X \in $In is a population in the
...
Mexican-hat function will be prefered.
...
population, casus an activation peaking at some column. This
...
element, which activation provides de output value $k_{\phi}$. Each
...
While the SF controller. keeps constant de $k_{\phi}=100$ value in
...
embedded mapping $k_{\phi}=M(v_n)$. As the ouput value of the neural
...
loss of stability (because the linear interpolation could understimate
...
controller converges faster to its fixed point (as would be spected
...
control policy (as parameterized by $k_{\phi}$) softly enough to
provide a qualitatively natural gait for the biped. It should be noted
that sudden changes of behaviour are common in Sliding-mode
...
to calculate te output $k_{\phi}$. 2) The neural field has natural
